...
db = pymysql.connect("localhost", "root", "123456", "shamir_key_one")
cursor = db.cursor()
...
async def handle_request(reader, writer):
    ...
    public = msg['public']
    sql = "SELECT * FROM apps_key_db_one WHERE productId = %s" % (int(productId))
    cursor.execute(sql)
    data = cursor.fetchone()
    encrypt_key = b2a_hex(
        encrypt(public, decrypt(keyring.get_password("DRMDEMO", "keyManagerOnePrivate"), a2b_hex(data[2])))).decode()
