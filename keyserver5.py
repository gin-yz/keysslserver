...
db = pymysql.connect("localhost", "root", "123456", "shamir_key_five")
cursor = db.cursor()
...
async def handle_request(reader, writer):
    ...
    public =msg['public']
    sql = "SELECT * FROM apps_key_db_five WHERE productId = %s" % (int(productId))
    cursor.execute(sql)
    data = cursor.fetchone()
    encrypt_key = b2a_hex(encrypt(public, decrypt(keyring.get_password("DRMDEMO", "keyManagerFivePrivate"), a2b_hex(data[2])))).decode()
    # print(encrypt_key)
    send_msg['status'] ='ok'
...
if __name__ == '__main__':
    ...
    server_sock.bind(('127.0.0.1', 7781))
